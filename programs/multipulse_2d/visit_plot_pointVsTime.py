...
if __name__ == '__main__':
    ...
    LineoutAtts.point2 = (0.0, 13.3e-3, 0.0)
    ...
    with tempfile.NamedTemporaryFile() as fname:
        it = 0
        for i in range(args.i0, args.i1, 2):
            print(fname.name, i)
            # Set output options
            ...
            t_values.append(v.GetQueryOutputValue())
            v.SaveWindow()

            line_data = np.genfromtxt(fname.name + '.curve')
            #var_values[it] = np.interp(args.r, line_data[:,0], line_data[:,1])
            var_values.append(np.interp(args.r, line_data[:,0], line_data[:,1]))
            it = it+1
    fname = args.database.split("*")[0]+args.varname+args.output+"_{pt}_pointVsTime.txt".format(pt=args.r)
    np.savetxt(fname, np.vstack([t_values, var_values]).T)
    print('Saved {file}'.format(file=fname))
    sys.exit()
